...
parser.add_option("-C", "--channel", dest="channel", default="NPDCCH", help="Channel  (PDCCH (for LTE), MPDCCH (for LTE-M) or NPDCCH (for NB-IoT)). Default is NPDCCH (NB-IoT)")
...
if channel not in ('NPDCCH','PDCCH', 'MPDCCH'):
...
if channel in ('NPDCCH',):
    ue_id_h_nbiot = hashed_id >> 20  # first 12 bits for NB-IOT
elif channel in ('PDCCH', 'MPDCCH'):
    ue_id_h_nbiot = hashed_id >> 22  # first 10 bits for LTE / LTE-M    
...
for i in range(20):
    date_hsfn_value = int((last_hsfn+paging_hyperframe+i*tedrx) % H_SFN_SIZE)
    date_hsfn = '{:4d}'.format(date_hsfn_value)
    date_hsfn_msb_8bits = '{:3d}'.format(date_hsfn_value >> 2)
    date_hsfn_bits = '{0:010b}'.format(date_hsfn_value)
    print('\t->', 'H-SFN:',date_hsfn, '('+date_hsfn_bits+')', '\tMSB 8 bits:', date_hsfn_msb_8bits, '\tdate:', initial_date + datetime.timedelta(seconds=last_tedrx_interval*tedrx_interval + i*tedrx_interval + paging_hyperframe*SFN_DURATION + ptw_start*FRAME_DURATION - leap_seconds))
print()